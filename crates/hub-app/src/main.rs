...
/// Window icon: the 256x256 straight-alpha RGBA generated by
/// `scripts/gen_icon.py`, embedded at compile time. egui 0.36's
/// `IconData` owns a `Vec<u8>` (no `Cow` variant), so the static bytes
/// are copied into the heap once at startup.
fn window_icon() -> egui::IconData {
    const RGBA: &[u8] = include_bytes!("../../../assets/icon-256.rgba");
    const SIZE: u32 = 256;
    egui::IconData {
        rgba: RGBA.to_vec(),
        width: SIZE,
